...
import type { UserRole } from '@/lib/helpers/rbac'
...
export default function UserManagementModal({ isOpen, onClose, editingUser, role, onSuccess }: UserManagementModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Check for duplicate email before creating/updating
      if (formData.email && formData.email.trim()) {
        ...
        // For non-super admins, check if email exists in their tenant
        if (!isSuper && tenantId) {
          ...
          if (tenantUsers && tenantUsers.length > 0) {
            ...
            emailCheckQuery = emailCheckQuery.in('id', userIds)
          } else {
            emailCheckQuery = emailCheckQuery.eq('id', '00000000-0000-0000-0000-000000000000') // No match
          }
        }
        
        const { data: existingUsers, error: emailCheckError } = await emailCheckQuery
        
        if (emailCheckError) {
          console.error('Error checking duplicate email:', emailCheckError)
          // Continue if check fails (don't block user)
        } else if (existingUsers && existingUsers.length > 0) {
          // If editing, allow if it's the same user
          if (editingUser && existingUsers.some((u: any) => u.id === editingUser.id)) {
            // Same user, email unchanged or changed to same value - allow
          } else {
            // Duplicate email found
            const existingUser = existingUsers[0]
            setError(`This email (${formData.email}) is already registered to another user: ${existingUser.name || existingUser.email}`)
            setIsLoading(false)
            return
          }
        }
