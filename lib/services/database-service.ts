...
class DatabaseService {
  ...
  private addTenantFilter(query: any, tenantId: string | null, isSuper: boolean = false) {
    ...
    // If no tenant_id and not super admin, let RLS handle it (will return empty if no access)
    return query
  }

  // Get current tenant ID - with fallback to database lookup
  ...
  async getRecentInvoices(limit: number = 5): Promise<Invoice[]> {
    try {
      ...
      if (invoicesError) {
        ...
        // Continue with empty array - will still try to get vehicle_inward data
      }

      const realInvoices: Invoice[] = (invoices || []) as Invoice[]
      ...
      inwardQuery = this.addTenantFilter(inwardQuery, tenantId, isSuper)
      const { data: allInwardEntriesRaw, error: inwardError } = await inwardQuery
      
      if (inwardError) {
        console.error('Error fetching vehicle_inward:', inwardError)
