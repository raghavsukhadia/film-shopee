...
function generateWhatsAppMessage(data: DailyReportWhatsAppData, totalVehicles: number): string {
  let message = `📋 *Daily Vehicle Report - ${data.reportDate}*\n\n`
  ...
  message += `📅 Vehicles for Tomorrow: *${data.nextDayCount}*\n`
  message += `⏳ Pending Vehicles: *${data.pendingCount}*\n`
  message += `📊 Total: *${totalVehicles}*\n\n`
