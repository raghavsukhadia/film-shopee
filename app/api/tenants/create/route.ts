...
export async function POST(request: Request) {
  try {
    ...
    // Generate tenant code (FS01, FS02, FS03, etc.)
    // Get the highest existing tenant code number
    const { data: existingTenants } = await adminSupabase
      .from('tenants')
      .select('tenant_code')
      .not('tenant_code', 'is', null)
      .or('tenant_code.like.FS%,tenant_code.like.Z%') // Support both FS and Z codes for migration
      .order('tenant_code', { ascending: false })
      .limit(10) // Get more to find the highest FS code
    
    let nextCode = 'FS01'
    if (existingTenants && existingTenants.length > 0) {
      // Find the highest FS code
      const fsCodes = existingTenants
        .map(t => t.tenant_code)
        .filter(code => code && code.match(/^FS(\d+)$/i))
        .map(code => parseInt(code!.substring(2)))
        .sort((a, b) => b - a)
      
      if (fsCodes.length > 0) {
        const lastNum = fsCodes[0]
        const nextNum = lastNum + 1
        nextCode = 'FS' + nextNum.toString().padStart(2, '0')
      }
    }
