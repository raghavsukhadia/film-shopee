...
function LoginContent() {
  ...
  const handleTenantLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Get tenant by tenant code (case-insensitive, supports FS, Z, and RS codes for backward compatibility)
      const normalizedCode = tenantCode.toUpperCase().trim()
      
      // Try to find tenant with the exact code first
      ...
      // If not found and code is FS01, try RS01 (for migration compatibility)
      if (!tenant && normalizedCode === 'FS01') {
        const { data: rsTenant, error: rsError } = await supabase
          .from('tenants')
          .select('id, name, workspace_url, tenant_code, is_active')
          .eq('tenant_code', 'RS01')
          .maybeSingle()
        ...
        if (rsTenant) {
          tenant = rsTenant
          tenantError = rsError
          // Show a helpful message that they should use RS01 or update their tenant
          console.warn('Found tenant with RS01 code. Please update tenant code to FS01 or use RS01 for login.')
        }
      }
      ...
      if (tenantError) {
        ...
        setError('Error finding tenant. Please try again or contact support.')
        setLoading(false)
        return
      }
      ...
      if (!tenant) {
        await supabase.auth.signOut()
        setError(`Tenant "${normalizedCode}" not found. Please check your tenant number (e.g., FS01, FS02, FS03).`)
        setLoading(false)
        return
      }
      ...
      // Use maybeSingle() instead of single() to handle cases where user might not be linked yet
      ...
      if (tenantUserError) {
        console.error('Error checking tenant user:', tenantUserError)
        await supabase.auth.signOut()
        setError('Error verifying tenant access. Please try again or contact support.')
        setLoading(false)
        return
      }

      if (!tenantUser) {
        // User is authenticated but not linked to this tenant
        // This can happen if user was created but tenant_users record wasn't created
        await supabase.auth.signOut()
    ...
    try {
      ...
      if (!isSuperAdmin && !isRSCarAdmin) {
        ...
        if (tenantUsers && tenantUsers.length > 0) {
          // Fetch latest tenant data directly from tenants table to ensure we have updated names
          const tenantIds = tenantUsers.map((tu: any) => tu.tenant_id).filter(Boolean)
          const { data: latestTenants } = await supabase
            .from('tenants')
            .select('id, name, workspace_url')
            .in('id', tenantIds)
          
          // Map tenant users with latest tenant data
          const tenants = tenantUsers.map((tu: any) => {
            // Find latest tenant data
            const latestTenant = latestTenants?.find((t: any) => t.id === tu.tenant_id)
            
            // Use latest data if available, otherwise fallback to joined data
            if (latestTenant) {
              return {
                id: latestTenant.id,
                name: latestTenant.name, // Use latest name from database
                workspace_url: latestTenant.workspace_url, // Use latest workspace_url from database
                role: tu.role
              }
            }
            
            // Fallback to joined data if direct fetch didn't work
      ...
      // Always fetch RS Car Accessories tenant directly from tenants table to get latest name
      const { data: rsCarTenantData, error: rsCarTenantError } = await supabase
        .from('tenants')
        .select('id, name, workspace_url')
        .eq('id', RS_CAR_ACCESSORIES_TENANT_ID)
        .single()
      
      // Add RS Car Accessories tenant with latest data from database
      if (rsCarTenantData && !rsCarTenantError) {
        // Get role from tenant_users if available
        const rsCarTenantUser = allTenantUsers?.find((tu: any) => tu.tenant_id === RS_CAR_ACCESSORIES_TENANT_ID)
        tenants.push({
          id: rsCarTenantData.id,
          name: rsCarTenantData.name, // Use latest name from database
          workspace_url: rsCarTenantData.workspace_url, // Use latest workspace_url from database
          role: rsCarTenantUser?.role || 'admin'
        })
      } else if (allTenantUsers) {
        // Fallback: try to get from tenant_users join if direct fetch fails
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              color: loginMode === 'tenant' ? '#f59e0b' : '#6b7280',
          ...
          <button
            ...
            style={{
              ...
              color: loginMode === 'super' ? '#f59e0b' : '#6b7280',
            ...
            {loginMode === 'tenant' && (
              <div style={{ marginBottom: '1rem' }}>
                ...
                <p style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '0.25rem' }}>
                  Enter your tenant number (e.g., FS01, FS02, FS03)
                </p>
              </div>
            )}
