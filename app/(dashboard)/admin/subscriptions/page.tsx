...
export default function SubscriptionManagementPage() {
  ...
  const loadSubscriptions = async () => {
    try {
      ...
      // Verify user is super admin (check both super_admins table and RS Car admin)
      ...
      console.log('Super admin verified, loading subscriptions data...')

      // Load tenants first (without relations to avoid RLS issues)
      let tenantsData: any[] = []
      try {
        const { data: tenantsResult, error: tenantsError } = await supabase
          .from('tenants')
          .select('id, name, tenant_code, subscription_status, is_active, trial_ends_at, is_free, created_at')
      ...
      const { data: subscriptions, error: subsError } = await supabase
        .from('subscriptions')
        .select('id, tenant_id, status, amount, billing_period_start, billing_period_end, trial_end_date')
      ...
      const { data: paymentProofs, error: proofError } = await supabase
        ...
        .select('id, tenant_id, transaction_id, payment_date, amount, currency, status, created_at, notes, payment_proof_url')
        .in('tenant_id', tenantIds)
        .order('created_at', { ascending: false })
      ...
      const { data: planRequests, error: planRequestsError } = await supabase
      ...
      if (planRequestsError) {
        console.warn('Error loading plan requests (non-critical):', planRequestsError)
      }

      // Group subscriptions and payment proofs by tenant_id
      ...
      const transformedSubscriptions = await Promise.all(
        tenantsData.map(async (tenant: any) => {
          ...
          const tenantPaymentProofs = paymentProofsByTenant[tenant.id] || []
          const sortedPaymentProofs = tenantPaymentProofs.sort((a: any, b: any) =>
          ...
          return {
            ...
            payment_proofs: sortedPaymentProofs,
