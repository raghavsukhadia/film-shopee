...
export default function TenantsManagementPage() {
  ...
  const loadTenants = async () => {
    try {
      ...
      // Verify user is super admin before loading tenants
      ...
      // First, try to load tenants without relations to avoid RLS issues
      let tenantsData: any[] = []
      
      // Load tenants first (simple query)
      ...
      if (tenantsError) {
        ...
        console.error('Full error object:', JSON.stringify(tenantsError, null, 2))
        console.error('User ID:', user.id)
        console.error('Is Super Admin:', !!superAdmin)
        console.error('Is RS Car Admin:', !!rsCarAdmin)
        setTenants([])
        return
      }
      ...
      // Load payment proofs separately to avoid RLS issues with relations
      if (tenantsData.length > 0) {
        const tenantIds = tenantsData.map(t => t.id)
        const { data: paymentProofs, error: proofError } = await supabase
          .from('tenant_payment_proofs')
          .select('id, tenant_id, status, created_at, transaction_id, payment_date, amount, currency, reviewed_at, payment_proof_url, admin_user_id')
        ...
        // Group payment proofs by tenant_id
        const paymentProofsByTenant: Record<string, any[]> = {}
        if (paymentProofs) {
          paymentProofs.forEach((proof: any) => {
            if (!paymentProofsByTenant[proof.tenant_id]) {
              paymentProofsByTenant[proof.tenant_id] = []
            }
            paymentProofsByTenant[proof.tenant_id].push(proof)
          })
        }

        // Attach payment proofs to tenants
        tenantsData = tenantsData.map((tenant: any) => ({
          ...tenant,
          tenant_payment_proofs: paymentProofsByTenant[tenant.id] || []
        }))
      }

      if (!tenantsData || tenantsData.length === 0) {
        console.warn('No tenants data returned')
        setTenants([])
        return
      }
      ...
      const subscriptionsByTenant: Record<string, any> = {}
      
      if (tenantIds.length > 0) {
        const { data: subscriptions, error: subsError } = await supabase
          .from('subscriptions')
          .select('id, tenant_id, status, amount, currency, billing_period_start, billing_period_end, plan_name, created_at')
          .in('tenant_id', tenantIds)
          .order('created_at', { ascending: false })
        ...
        if (subsError) {
          ...
          // Get the latest subscription for each tenant
          subscriptions.forEach((sub: any) => {
            if (!subscriptionsByTenant[sub.tenant_id]) {
              subscriptionsByTenant[sub.tenant_id] = sub
            }
          })
        }
      }
      ...
      const transformedTenants = tenantsData.map((tenant: any) => {
        ...
        return {
          ...
          subscription: subscriptionsByTenant[tenant.id] || null,
