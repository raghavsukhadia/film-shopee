...
export default function SettingsPageClient() {
  ...
  const [activeTab, setActiveTab] = useState('company') // Default to company, will be adjusted based on role
  ...
  const [locationForm, setLocationForm] = useState({ name: '', address: '', is_active: true })
  ...
  // - Admin: All tabs (Profile, Company, Management, Notifications, Payment)
  // - Manager: Company, Management, Notifications (no Profile, no Payment)
  // - Coordinator, Installer, Accountant: Company only (no Profile)
  const filteredTabs = userRole === 'admin'
    ? tabs // Admin sees all tabs
    : userRole === 'manager'
    ? tabs.filter(t => t.id !== 'payment' && t.id !== 'profile') // Manager sees all except Payment and Profile
    : tabs.filter(t => t.id === 'company') // Other roles see only Company (no Profile)
  ...
  const loadCurrentUser = async () => {
    try {
      ...
      if (user) {
        ...
        // Also check if user is admin in the first tenant (Z01) - which is the super admin tenant
        ...
        if (!isTenantAdmin) {
          ...
          if (z01Admin) {
            isZ01Admin = true
            isTenantAdmin = true // Treat Z01 admin as tenant admin
          }
        }
        ...
        const effectiveRole = (isSuperAdmin || isTenantAdmin) ? 'admin' : (profile?.role || 'coordinator')
        setUserRole(effectiveRole as any)
        
        // Only redirect to company tab for non-admin roles
        if (effectiveRole !== 'admin' && (effectiveRole === 'installer' || effectiveRole === 'coordinator' || effectiveRole === 'manager' || effectiveRole === 'accountant')) {
  ...
  const fetchSupportEmails = async () => {
    try {
      ...
      logger.error('Error fetching support emails', error)
    } finally {
  ...
  const checkActivationRequestStatus = async () => {
    try {
      ...
      logger.error('Error checking activation request status', error)
    }
  }
  ...
  useEffect(() => {
    ...
    // Redirect non-admin users away from profile tab
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (!isSuper && tenantId) {
        ...
        // No tenant and not super admin - set empty array
        setUsers([])
      }
    } catch (error) {
  ...
  const fetchInstallers = async () => {
    try {
      ...
      if (!isSuper && tenantId) {
        ...
        if (tenantUsersError) {
          logger.error('Error fetching tenant users for installers', tenantUsersError)
          setInstallersList([])
          return
        }
        ...
        if (tenantUsers && tenantUsers.length > 0) {
          ...
          if (error) {
            logger.error('Error fetching installer profiles', error)
            setInstallersList([])
            return
          }
        ...
        const { data, error } = await supabase
          ...
          .order('created_at', { ascending: false })
        if (error) {
          logger.error('Error fetching all installers', error)
          setInstallersList([])
          return
        }
        setInstallersList(data || [])
  ...
  const fetchManagers = async () => {
    try {
      ...
      if (!isSuper && tenantId) {
        ...
        if (tenantUsersError) {
          logger.error('Error fetching tenant users for managers', tenantUsersError)
          setManagers([])
          return
        }
        ...
        if (tenantUsers && tenantUsers.length > 0) {
          ...
          const { data, error } = await supabase
            ...
            .order('created_at', { ascending: false })
          if (error) {
            logger.error('Error fetching manager profiles', error)
            setManagers([])
            return
          }
          setManagers(data || [])
        ...
        const { data, error } = await supabase
          ...
          .order('created_at', { ascending: false })
        if (error) {
          logger.error('Error fetching all managers', error)
          setManagers([])
          return
        }
        setManagers(data || [])
  ...
  const fetchAccountants = async () => {
    try {
      ...
      if (!isSuper && tenantId) {
        ...
        if (tenantUsersError) {
          logger.error('Error fetching tenant users for accountants', tenantUsersError)
          setAccountantsList([])
          return
        }
        ...
        if (tenantUsers && tenantUsers.length > 0) {
          ...
          const { data, error } = await supabase
            ...
            .order('created_at', { ascending: false })
          if (error) {
            logger.error('Error fetching accountant profiles', error)
            setAccountantsList([])
            return
          }
        ...
        const { data, error } = await supabase
          ...
          .order('created_at', { ascending: false })
        if (error) {
          logger.error('Error fetching all accountants', error)
          setAccountantsList([])
          return
        }
        setAccountantsList(data || [])
      ...
      logger.error('Error fetching accountants', error)
      setAccountantsList([])
    }
  }
  ...
  const fetchMessageTemplates = async () => {
    try {
      ...
      if (!tenantId) {
        logger.warn('No tenant ID found, using default templates')
      ...
      if (error) {
        // If table doesn't exist, RLS error, or permission denied, use default templates
        if (
          error.code === '42P01' || 
          error.code === '42501' ||
          ...
          logger.warn('Error fetching message templates (using defaults)', { code: error.code, message: error.message })
      ...
      logger.error('Exception fetching message templates', error)
    ...
    try {
      ...
      for (const [eventType, template] of templateContent.entries()) {
        try {
          ...
          if (error) {
            logger.error(`Error saving template ${eventType}`, error)
            errors.push(`${eventType}: ${error.message || 'Unknown error'}`)
          } else {
            savedCount++
          }
        } catch (err: any) {
          logger.error(`Exception saving template ${eventType}`, err)
      ...
      logger.error('Error saving message templates', error)
  ...
  const saveWhatsappSettings = async () => {
    try {
      ...
      for (const setting of settings) {
        ...
        if (!tenantId && !isSuper) {
          logger.error('Cannot save WhatsApp settings: tenantId is required for tenant users')
        ...
        let query = supabase
          ...
          // Note: setting_group column may not exist yet, filter by key only
        ...
        if (queryError) {
          logger.error('Error checking existing WhatsApp setting', queryError)
          throw queryError
        }
        ...
        if (existing) {
          ...
          if (tenantId && !isSuper && existing.tenant_id !== tenantId) {
            logger.error('Security violation: Attempted to update WhatsApp setting from different tenant', undefined, {
          ...
          if (insertError) {
            logger.error('Error inserting WhatsApp setting', insertError, { insertData })
            throw insertError
          }
        }
      }
  ...
  const fetchSystemSettings = async () => {
    try {
      ...
      let query = supabase
        .from('system_settings')
        .select('*')
        .or('setting_key.like.profile_%,setting_key.like.company_%')
      ...
      if (data && data.length > 0) {
        ...
        data.forEach(setting => {
          if (setting.setting_key?.startsWith('profile_')) {
            const key = setting.setting_key.replace('profile_', '')
            profileData[key] = setting.setting_value
          } else if (setting.setting_key?.startsWith('company_')) {
  ...
  // Helper function to generate workspace URL slug from company name
  const generateWorkspaceUrl = (companyName: string): string => {
    ...
    let slug = companyName
      .toLowerCase()
      .trim()
      // Remove special characters like bullet points (•)
      .replace(/[•·]/g, '-')
  ...
  const saveCompanySettings = async () => {
    ...
    try {
      ...
      // Get current tenant data to check workspace_url
      ...
      // Generate new workspace URL from company name if name changed
      let newWorkspaceUrl: string | null = null
      if (tenantId && companySettings.name && companySettings.name.trim()) {
        // Only update workspace_url if company name has changed
        if (!currentTenant || currentTenant.name !== companySettings.name) {
          newWorkspaceUrl = generateWorkspaceUrl(companySettings.name)
          
          // Check if workspace_url already exists for another tenant
          if (newWorkspaceUrl) {
            const { data: existingTenant } = await supabase
              .from('tenants')
              .select('id')
              .eq('workspace_url', newWorkspaceUrl)
              .neq('id', tenantId)
              .single()
            
            // If workspace_url exists, append tenant code or make it unique
            if (existingTenant) {
              // Get tenant code to make it unique
              ...
              if (tenantData?.tenant_code) {
                newWorkspaceUrl = `${newWorkspaceUrl}-${tenantData.tenant_code.toLowerCase()}`
              } else {
                // Fallback: append random suffix
                newWorkspaceUrl = `${newWorkspaceUrl}-${Date.now().toString().slice(-4)}`
              }
            }
          }
        }
      ...
      // Update tenant name and workspace_url if changed
      if (tenantId && !isSuper) {
        const updateData: any = { name: companySettings.name }
        
        // Only update workspace_url if it was generated (name changed)
        if (newWorkspaceUrl) {
          updateData.workspace_url = newWorkspaceUrl
        }
        ...
        // Super admin can also update tenant info
        const updateData: any = { name: companySettings.name }
        ...
        if (tenantUpdateError) {
          logger.error('Error updating tenant', tenantUpdateError)
          throw tenantUpdateError
        }
        
        // Update sessionStorage with new workspace URL if it was changed
        if (newWorkspaceUrl && typeof window !== 'undefined') {
          sessionStorage.setItem('current_workspace_url', newWorkspaceUrl)
        }
      }
      ...
      for (const setting of settings) {
        ...
        // For tenant users, tenantId must be set and not null
        if (!tenantId && !isSuper) {
          logger.error('Cannot save settings: tenantId is required for tenant users')
          throw new Error('Tenant ID is required to save settings')
        }
        ...
        if (queryError) {
          logger.error('Error checking existing setting', queryError)
          throw queryError
        }
        ...
        if (existing) {
          ...
          if (insertError) {
            logger.error('Error inserting setting', insertError, { insertData })
            throw insertError
          }
        }
      }
  ...
  const handleToggleStatus = async (id: string, type: string) => {
    try {
      ...
      // Convert is_active boolean to new status
      // For locations, vehicle_types, and departments, use is_active boolean
      // For profiles, use status string
      let updateData: any
      if (table === 'locations' || table === 'vehicle_types' || table === 'departments') {
        const currentIsActive = currentItem.is_active !== false // Default to true if undefined
    ...
    try {
      ...
      if (editingVehicleType) {
        ...
        if (!isSuper && tenantId) {
          ...
          const { error } = await supabase
            .from('vehicle_types')
            .update({ name: vehicleTypeForm.name, is_active: vehicleTypeForm.is_active })
            .eq('id', editingVehicleType.id)
            .eq('tenant_id', tenantId) // Extra security filter
      ...
      setVehicleTypeForm({ name: '', is_active: true })
    } catch (error: any) {
      alert(`Failed to save: ${error.message}`)
    }
  }
  ...
  const handleSaveLocation = async () => {
    ...
    try {
      ...
      setLocationForm({ name: '', address: '', is_active: true })
    } catch (error: any) {
      alert(`Failed to save: ${error.message}`)
    }
  }
    ...
    <>
      ...
      <div style={{ 
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
                ...
                <div style={{ backgroundColor: 'white', borderRadius: '0.5rem', padding: '1.5rem', marginBottom: '1rem', border: '1px solid #e2e8f0' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem', marginBottom: '1rem' }}>
                    ...
                    <div>
                      ...
                      <select
                        value={locationForm.is_active ? 'active' : 'inactive'}
                        onChange={(e) => setLocationForm({ ...locationForm, is_active: e.target.value === 'active' })}
                        style={{
                          width: '100%',
                          padding: '0.75rem',
                          border: '1px solid #e2e8f0',
                        ...
                        <tr key={location.id} style={{ borderBottom: index === locations.length - 1 ? 'none' : '1px solid #f1f5f9' }}>
                          ...
                          <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem' }}>
                            <span style={{
                              ...
                              fontSize: '0.75rem',
                              fontWeight: '500',
                              backgroundColor: (location.is_active !== false) ? '#dcfce7' : '#fef2f2',
                              color: (location.is_active !== false) ? '#166534' : '#dc2626'
                            }}>
                          ...
                          <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem' }}>
                            <div style={{ display: 'flex', gap: '0.5rem' }}>
                              ...
                              <button
                                ...
                                style={{
                                  padding: '0.25rem 0.5rem',
                                  backgroundColor: (location.is_active !== false) ? '#f59e0b' : '#059669',
                                  color: 'white',
                                  border: 'none',
                                  borderRadius: '0.25rem',
                                  fontSize: '0.75rem',
                ...
                <div style={{ backgroundColor: 'white', borderRadius: '0.5rem', padding: '1.5rem', marginBottom: '1rem', border: '1px solid #e2e8f0' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem', marginBottom: '1rem' }}>
                    ...
                    <div>
                      ...
                      <select
                        value={vehicleTypeForm.is_active ? 'active' : 'inactive'}
                        onChange={(e) => setVehicleTypeForm({ ...vehicleTypeForm, is_active: e.target.value === 'active' })}
                        style={{
                          width: '100%',
                          padding: '0.75rem',
                          border: '1px solid #e2e8f0',
                        ...
                        <tr key={vehicleType.id} style={{ borderBottom: index === vehicleTypes.length - 1 ? 'none' : '1px solid #f1f5f9' }}>
                          ...
                          <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem' }}>
                            <span style={{
                              ...
                              fontSize: '0.75rem',
                              fontWeight: '500',
                              backgroundColor: (vehicleType.is_active !== false) ? '#dcfce7' : '#fef2f2',
                              color: (vehicleType.is_active !== false) ? '#166534' : '#dc2626'
                            }}>
                              {(vehicleType.is_active !== false) ? 'Active' : 'Inactive'}
                            </span>
                          </td>
                          <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem', color: '#1e293b' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem', marginBottom: '1rem' }}>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setDepartmentForm({ ...departmentForm, is_active: e.target.value === 'active' })}
                        style={{
                          width: '100%',
                          padding: '0.75rem',
                          border: '1px solid #e2e8f0',
                        ...
                        <tr key={department.id} style={{ borderBottom: index === departments.length - 1 ? 'none' : '1px solid #f1f5f9' }}>
                          ...
                          <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem' }}>
                            <span style={{
                              ...
                              fontSize: '0.75rem',
                              fontWeight: '500',
                              backgroundColor: (department.is_active !== false) ? '#dcfce7' : '#fef2f2',
                              color: (department.is_active !== false) ? '#166534' : '#dc2626'
                            }}>
                            ...
                            <div style={{ display: 'flex', gap: '0.5rem' }}>
                              <button
                                onClick={() => {
                                  ...
                                  setDepartmentForm({ name: department.name || '', is_active: department.is_active !== false, color: department.color || '#3b82f6' })
                                  setShowDepartmentForm(true)
                                }}
                                style={{
                                  padding: '0.25rem 0.5rem',
                              ...
                              <button
                                ...
                                style={{
                                  padding: '0.25rem 0.5rem',
                                  backgroundColor: (department.is_active !== false) ? '#f59e0b' : '#059669',
                                  color: 'white',
                                  border: 'none',
                                  borderRadius: '0.25rem',
                                  fontSize: '0.75rem',
          ...
          {/* Profile Settings - Only visible to Admin */}
                    ...
                    <div style={{ fontSize: '0.9375rem', color: '#374151' }}>
                      {subscriptionStatus === 'trial' && trialEndsAt && new Date(trialEndsAt) > new Date() ? (
                        ...
                        <span style={{ 
                          ...
                          <XCircle style={{ width: '0.875rem', height: '0.875rem', display: 'inline', marginRight: '0.25rem' }} />
