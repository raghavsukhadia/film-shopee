  ...
  const fetchInstallers = async () => {
    try {
      ...
      if (!isSuper && tenantId) {
        ...
        if (tenantUsersError) {
          console.error('Error fetching tenant users for installers:', tenantUsersError)
          setInstallersList([])
          return
        }
        ...
        if (tenantUsers && tenantUsers.length > 0) {
          ...
          if (error) {
            console.error('Error fetching installer profiles:', error)
            setInstallersList([])
            return
          }
  ...
  const fetchAccountants = async () => {
    try {
      ...
      if (!isSuper && tenantId) {
        ...
        if (tenantUsersError) {
          console.error('Error fetching tenant users for accountants:', tenantUsersError)
          setAccountantsList([])
          return
        }
        ...
        if (tenantUsers && tenantUsers.length > 0) {
          ...
          const { data, error } = await supabase
            ...
            .order('created_at', { ascending: false })
          if (error) {
            console.error('Error fetching accountant profiles:', error)
            setAccountantsList([])
            return
          }
