...
import { exportToExcel, exportToExcelMultiSheet, formatCurrency, formatDate, type ExcelColumn } from '@/lib/excel-export'
...
export default function AccountsPageClient() {
  ...
  const fetchCompletedEntries = async () => {
    try {
      ...
      // Sort by created_at descending - newest entries first
      let query = supabase
        .from('vehicle_inward')
        .select('*')
  ...
  const fetchAccountEntries = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        logger.info(`Fetched ${mappedEntries.length} account entries (all active vehicles)`)
        setEntries(mappedEntries)
      } else {
        logger.info('No active vehicles found')
        setEntries([])
      }
    } catch (error) {
      logger.error('Error fetching account entries', error)
  ...
  const handleExportToExcel = () => {
    try {
      ...
      if (dataToExport.length === 0) {
        alert('No entries to export. Please ensure there are entries in the current view.')
        return
      }
      ...
      alert(`Successfully exported ${dataToExport.length} entry/entries to Excel!`)
    } catch (error: any) {
  ...
  const handleAddInvoiceLink = async () => {
    ...
    try {
      ...
      if (!user) {
        alert('You must be logged in to add invoice links')
        return
      }
      ...
      alert(`Failed to add invoice link: ${error.message || 'Unknown error'}`)
    } finally {
  ...
  const handleUploadInvoice = async () => {
    ...
    try {
      ...
      if (!user) {
        alert('You must be logged in to upload invoices')
        setInvoiceLoading(false)
        return
      }
  ...
  const handleSaveInvoiceNumber = async () => {
    ...
    try {
      ...
      logger.error('Error saving invoice number', error)
      alert(`Failed to save invoice number: ${error.message || 'Unknown error'}`)
    } finally {
                  ...
                  <button
                    onClick={() => {
                      ...
                      try {
                        ...
                        exportToExcel({
                          filename: `completed_entries_${periodLabel}_${new Date().toISOString().split('T')[0]}`,
                          sheetName: 'Completed Entries',
                          title: 'Completed Account Entries Export',
                          subtitle: `Total Entries: ${filteredCompleted.length} | Period: ${timeFilter === 'all' ? 'All Time' : timeFilter}`,
                          columns,
                          data: filteredCompleted,
                          includeDate: true
                        })

                        alert(`Successfully exported ${filteredCompleted.length} entry/entries to Excel!`)
                      } catch (error: any) {
        ...
        >
          ...
          >
            ...
            <div style={{ padding: '1.5rem' }}>
              ...
              {/* Invoice Number - Add/Edit Section */}
              <div style={{ marginBottom: '2rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                  <h3 style={{ fontSize: '1.125rem', fontWeight: '600', color: '#111827', display: 'flex', alignItems: 'center', gap: '0.5rem', margin: 0 }}>
                  ...
                  {!editingInvoiceNumber && selectedEntry.invoiceNumber && (
                    <button
                ...
                <div style={{ 
                  backgroundColor: selectedEntry.invoiceNumber ? '#f0f9ff' : '#f9fafb', 
                  borderRadius: '0.75rem', 
                  padding: '1.25rem',
                  border: selectedEntry.invoiceNumber ? '1px solid #bae6fd' : '1px solid #e5e7eb'
                }}>
                  {editingInvoiceNumber || !selectedEntry.invoiceNumber ? (
                    <div>
                      <label style={{ 
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            backgroundColor: savingInvoiceNumber ? '#9ca3af' : '#059669',
                            color: 'white',
                            border: 'none',
                            borderRadius: '0.5rem',
                            fontSize: '0.875rem',
                            fontWeight: '600',
                            cursor: savingInvoiceNumber ? 'not-allowed' : 'pointer',
                            transition: 'all 0.2s'
                          }}
                          onMouseEnter={(e) => {
                            if (!savingInvoiceNumber) {
                              e.currentTarget.style.backgroundColor = '#047857'
                            }
                          }}
                          onMouseLeave={(e) => {
                        ...
                        >
                          <Save style={{ width: '1rem', height: '1rem' }} />
                          {savingInvoiceNumber ? 'Saving...' : 'Save'}
                        </button>
                        {(editingInvoiceNumber || !selectedEntry.invoiceNumber) && (
                          <button
                            onClick={() => {
              ...
              <div style={{ display: 'flex', gap: '1rem', justifyContent: 'space-between', alignItems: 'center', paddingTop: '1rem', borderTop: '2px solid #e5e7eb' }}>
                {/* Mark Complete Button - Show for all entries that are not already completed/delivered */}
                {(() => {
                  const status = (selectedEntry.status || '').toLowerCase().trim()
                  const isCompleted = ['completed', 'complete_and_delivered', 'delivered', 'delivered_final'].includes(status)
                  return !isCompleted && (
                    <button
                  ...
                  <button
                    onClick={() => {
                      try {
                        ...
                        // Export entry details with products
                        const productData = selectedEntry.products.map((product: any) => ({
                          product: product.product,
                          brand: product.brand,
                          department: departmentNames.get(product.department) || product.department,
                          price: formatCurrency(product.price)
                        ...
                        exportToExcelMultiSheet(
                          `entry_${selectedEntry.shortId || selectedEntry.id.substring(0, 8)}_${new Date().toISOString().split('T')[0]}`,
                          [
                            {
                              name: 'Entry Details',
                              title: `Account Entry: ${selectedEntry.shortId || selectedEntry.id.substring(0, 8)}`,
                              subtitle: `Customer: ${selectedEntry.customerName} | Vehicle: ${selectedEntry.vehicleNumber}`,
                              columns: [
                                { header: 'Field', key: 'field', width: 25 },
                                { header: 'Value', key: 'value', width: 30 }
                              ],
                              data: [
                                { field: 'Entry ID', value: selectedEntry.shortId || selectedEntry.id.substring(0, 8) },
                                { field: 'Customer Name', value: selectedEntry.customerName },
                                { field: 'Phone', value: selectedEntry.customerPhone },
                                { field: 'Email', value: selectedEntry.customerEmail || 'N/A' },
                                { field: 'Vehicle Number', value: selectedEntry.vehicleNumber },
                                { field: 'Model', value: selectedEntry.model },
                                { field: 'Make', value: selectedEntry.make },
                                { field: 'Year', value: selectedEntry.year?.toString() || 'N/A' },
                                { field: 'Color', value: selectedEntry.color || 'N/A' },
                                { field: 'Vehicle Type', value: selectedEntry.vehicleType ? (vehicleTypeNames.get(selectedEntry.vehicleType) || selectedEntry.vehicleType) : 'N/A' },
                                { field: 'Location', value: selectedEntry.location ? (locationNames.get(selectedEntry.location) || selectedEntry.location) : 'N/A' },
                                { field: 'Manager', value: selectedEntry.manager ? (managerNames.get(selectedEntry.manager) || selectedEntry.manager) : 'N/A' },
                                { field: 'Installation Complete Date', value: formatDate(selectedEntry.installationCompleteDate) },
                                { field: 'Expected Delivery', value: selectedEntry.expectedDelivery ? formatDate(selectedEntry.expectedDelivery) : 'N/A' },
                                { field: 'Total Amount', value: formatCurrency(selectedEntry.totalAmount) },
                                { field: 'Status', value: selectedEntry.status },
                                ...(selectedEntry.status === 'completed' ? [
                                  { field: 'Discount Amount', value: formatCurrency(selectedEntry.discountAmount || 0) },
                                  { field: 'Discount %', value: selectedEntry.discountPercentage ? `${selectedEntry.discountPercentage.toFixed(2)}%` : '0%' },
                                  { field: 'Discount Offered By', value: selectedEntry.discountOfferedBy || 'N/A' },
                                  { field: 'Discount Reason', value: selectedEntry.discountReason || 'N/A' },
                                  { field: 'Final Amount', value: formatCurrency(selectedEntry.finalAmount || selectedEntry.totalAmount) }
                                ] : [])
                              ]
                            },
                            {
                              ...
                              data: productData.length > 0 ? productData : [{ product: 'No products', brand: '', department: '', price: '' }]
                            }
                          ]
                        )

                        alert('Entry exported to Excel successfully!')
                      } catch (error: any) {
