...
export default function RequirementsPageClient() {
  ...
  const [requirementAttachments, setRequirementAttachments] = useState<any[]>([]) // Attachments for the requirement itself
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '42703' && error.message?.includes('tenant_id')) {
          logger.error('ERROR: tenant_id column is missing in customer_requirements table. SOLUTION: Please run this SQL migration in Supabase SQL Editor: File: database/add_tenant_id_to_customer_requirements.sql. This will add the tenant_id column and enable multi-tenant data isolation.')
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (selectedRequirement?.editing) {
        ...
        // Prepare insert data - map description to requirement if needed
        const insertData: any = {
          customer_name: formData.customer_name,
          customer_number: formData.customer_number,
          vehicle_name: formData.vehicle_name,
          description: formData.description,
          requirement: formData.description || formData.requirement || '', // Map description to requirement for backward compatibility
          priority: formData.priority,
          status: formData.status,
          tenant_id: tenantId
        }
        
        logger.debug('Inserting customer requirement', { data: insertData })
  ...
  const fetchRequirementAttachments = async (requirementId: string) => {
    try {
      ...
      if (error) {
        logger.error('Error fetching requirement attachments', error)
        setRequirementAttachments([])
        return
      }

      setRequirementAttachments(data || [])
    } catch (error: any) {
      logger.error('Error fetching requirement attachments', error)
      setRequirementAttachments([])
    }
  ...
  const handleOpenComments = async (requirement: any) => {
    ...
    await fetchRequirementAttachments(requirement.id)
  }
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      if (commentError) {
        logger.error('Error inserting comment', commentError)
        throw commentError
      }
            ...
            {requirementAttachments.length > 0 && (
              <div style={{ marginBottom: '2rem', padding: '1.5rem', backgroundColor: '#f0f9ff', borderRadius: '0.5rem', border: '1px solid #bae6fd' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
                  <Upload style={{ width: '1.5rem', height: '1.5rem', color: '#2563eb' }} />
                  <h3 style={{ fontSize: '1rem', fontWeight: '600', color: '#1e293b', margin: 0 }}>
                    ↑ Requirement Files ({requirementAttachments.length})
                  </h3>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {requirementAttachments.map((attachment, index) => (
                    <div key={index} style={{
                      display: 'flex',
                      alignItems: 'center',
                      gap: '0.75rem',
                      ...
                      <FileText style={{ width: '1.25rem', height: '1.25rem', color: '#64748b', flexShrink: 0 }} />
