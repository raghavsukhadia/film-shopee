...
export default function DashboardPageClient() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [kpisData, vehiclesData, invoicesData] = await Promise.all([
        dbService.getDashboardKPIs().catch(err => {
          ...
          return {
            ...
            overduePayments: 0,
            monthlyRevenue: 0,
            vehiclesInWorkshopChange: 0,
            jobsInProgressChange: 0,
            todaysIntakesChange: 0,
            unpaidInvoicesChange: 0,
            overduePaymentsChange: 0,
            monthlyRevenueChange: 0
          }
        }),
        dbService.getRecentVehicles(5).catch(err => {
          logger.error('Error loading vehicles', err)
          return []
        }),
        dbService.getRecentInvoices(5).catch(err => {
      ...
      // Generate sequential IDs for vehicles
      // First, fetch all vehicles to calculate correct sequential IDs
      const supabase = createClient()
      const tenantId = getCurrentTenantId()
      const isSuper = isSuperAdmin()
      
      let allVehiclesQuery = supabase
        .from('vehicle_inward')
        .select('id, created_at')
        .order('created_at', { ascending: true })
      
      if (!isSuper && tenantId) {
        allVehiclesQuery = allVehiclesQuery.eq('tenant_id', tenantId)
      }
      
      const { data: allVehicles } = await allVehiclesQuery
      
      // Create a map of vehicle ID to sequential ID
      const vehicleIdToSequentialId = new Map<string, string>()
      if (allVehicles) {
        allVehicles.forEach((v: any, index: number) => {
          const sequentialId = `Z${String(index + 1).padStart(2, '0')}`
          vehicleIdToSequentialId.set(v.id, sequentialId)
        })
      }
      
      // Add sequential IDs to recent vehicles
      const vehiclesWithSequentialIds = (vehiclesData || []).map((v: any) => ({
        ...v,
        shortId: vehicleIdToSequentialId.get(v.id) || v.short_id || v.id.substring(0, 8)
      }))
      
      setRecentVehicles(vehiclesWithSequentialIds)
  ...
  const loadInvoiceNumbers = async () => {
    try {
      ...
      logger.error('Error loading invoice numbers', error)
    }
  }
