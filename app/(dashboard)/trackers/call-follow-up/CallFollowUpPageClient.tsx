...
export default function CallFollowUpPageClient() {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '42703' && error.message?.includes('tenant_id')) {
          logger.error('ERROR: tenant_id column is missing in call_follow_up table. SOLUTION: Please run this SQL migration in Supabase SQL Editor: File: database/add_tenant_id_to_call_follow_up.sql. This will add the tenant_id column and enable multi-tenant data isolation.')
  ...
  const handleSubmit = async () => {
    try {
      ...
      logger.error('Error saving call', error)
