...
export default function ServiceTrackerPageClient() {
  ...
  const fetchServices = async () => {
    try {
      ...
      let query = supabase
        .from('service_trackers')  // Correct table name (plural)
        .select(`
          *,
          vehicle:vehicles(*),
          work_order:work_orders(*)
        `)
        .order('created_at', { ascending: false })  // Use created_at instead of scheduled_date
      ...
      // Calculate attachments_count and comments_count for each service
      if (data && data.length > 0) {
        const serviceIds = data.map(s => s.id)
        
        // Fetch all attachments for these services
        const { data: attachmentsData } = await supabase
          .from('service_tracker_attachments')
          .select('service_tracker_id')
          .in('service_tracker_id', serviceIds)
        
        // Fetch all comments for these services
        const { data: commentsData } = await supabase
          .from('service_tracker_comments')
          .select('service_tracker_id')
          .in('service_tracker_id', serviceIds)
        
        // Count attachments per service
        const attachmentsCountMap: {[key: string]: number} = {}
        attachmentsData?.forEach(att => {
          const serviceId = att.service_tracker_id
          attachmentsCountMap[serviceId] = (attachmentsCountMap[serviceId] || 0) + 1
        })
        
        // Count comments per service
        const commentsCountMap: {[key: string]: number} = {}
        commentsData?.forEach(comment => {
          const serviceId = comment.service_tracker_id
          commentsCountMap[serviceId] = (commentsCountMap[serviceId] || 0) + 1
        })
        
        // Add attachments_count and comments_count to each service
        const servicesWithCounts = data.map(service => ({
          ...service,
          attachments_count: attachmentsCountMap[service.id] || 0,
          comments_count: commentsCountMap[service.id] || 0
        }))
  ...
  const fetchComments = async (serviceId: string) => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        // Create a map of comment_id -> attachments and count them
        ...
        attachmentsData?.forEach(att => {
          ...
          if (!attachmentsMap[commentId]) {
            attachmentsMap[commentId] = []
          }
          attachmentsMap[commentId].push(att)
          attachmentsCountMap[commentId] = (attachmentsCountMap[commentId] || 0) + 1
        })
        
        setCommentAttachmentsMap(attachmentsMap)
        
        // Update comments with actual attachments_count and sync to database if needed
        const updatedComments = data.map(comment => {
          const actualCount = attachmentsCountMap[comment.id] || 0
          const storedCount = comment.attachments_count || 0
          
          // If count is out of sync, update it in the database
          if (actualCount !== storedCount) {
            supabase
              .from('service_tracker_comments')
              .update({ attachments_count: actualCount })
              .eq('id', comment.id)
              .then(({ error }) => {
                if (error) {
                  logger.error('Error updating attachments_count for comment', error, { commentId: comment.id })
                }
              })
          }
  ...
  const handleFileUpload = async (serviceId: string) => {
    ...
    try {
      ...
      for (const file of selectedFiles) {
        ...
        // Get tenant_id from the service_tracker record to ensure it matches
        ...
        if (serviceError) {
          console.error('Error fetching service_tracker:', serviceError)
        }

        // Get tenant_id - prefer from service_tracker, fallback to session storage
        let tenantId = serviceData?.tenant_id || getCurrentTenantId()
        
        // If still no tenant_id, try to get it from the user's tenant_users record
        if (!tenantId) {
          ...
          if (user) {
            ...
            if (tenantUser?.tenant_id) {
              ...
              console.log('Got tenant_id from tenant_users:', tenantId)
            }
          }
        }
        ...
        console.log('Inserting attachment with tenant_id:', tenantId, 'for service:', serviceId)
        ...
        // Only add tenant_id if we have it
        if (tenantId) {
        ...
        const { error: dbError, data: insertResult } = await supabase
          .from('service_tracker_attachments')
          .insert(insertData)
          .select()
      ...
      // Note: attachments_count column doesn't exist in service_trackers table schema
      // Commenting out to prevent errors - can be added via migration if needed
      // await supabase
      //   .from('service_trackers')
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      let createdByName = user.email || 'User'
      ...
      // If no tenant_id, try to get it from the service_tracker record
      let finalTenantId = tenantId
      if (!finalTenantId && selectedService?.id) {
      ...
      if (selectedCommentFiles.length > 0) {
        for (const file of selectedCommentFiles) {
          ...
          const attachmentInsertData: any = {
          ...
          // Add tenant_id if we have it
          if (finalTenantId) {
            attachmentInsertData.tenant_id = finalTenantId
          }
          ...
          if (attachmentError) {
            console.error('Error inserting comment attachment:', attachmentError)
        ...
        // Update attachments_count with actual number of successfully uploaded files
        await supabase
          .from('service_tracker_comments')
          .update({ attachments_count: successfullyUploadedCount })
          .eq('id', commentData.id)
      }

      // Note: comments_count column doesn't exist in service_trackers table schema
      // Commenting out to prevent errors - can be added via migration if needed
      // await supabase
      //   .from('service_trackers')
  ...
  const filteredServices = services.filter(service => {
    const matchesSearch = service.modal_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          ...
                          service.service_center?.toLowerCase().includes(searchTerm.toLowerCase())
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead style={{ backgroundColor: '#f8fafc' }}>
            <tr style={{ borderBottom: '1px solid #e2e8f0' }}>
              ...
              <th style={{ padding: '1rem', textAlign: 'left', fontSize: '0.875rem', fontWeight: '600', color: '#475569' }}>SERVICE CENTER</th>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('service_center', e.target.value)}
